...
- `../migrate-job.yaml.example`: Kubernetes 초기화 Job 예시.
...
```bash
uv run --no-sync --env-file .env alembic upgrade head --sql
```
...
초기화 도구였다. 현재 CLI와 schema.sql은 삭제되었고, 잔여 Store.migrate()는
호출하면 실패하므로 사용하지 않는다. 이후 스키마 변경은 Alembic으로만 관리한다.
...
```bash
alembic revision -m "describe worker schema change"
```
...
legacy 동등성 테스트는 삭제된 SQL을 참조해 현재 선별 회귀에서 제외한다.
현재 결과와 과거 전체 통과 기록의 구분은 [검증 매핑](../VALIDATION.md)에 있다.
