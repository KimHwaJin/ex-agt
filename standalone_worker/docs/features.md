...
**현재는 일부 운영 도구를 삭제한 뒤 참조 정리가 남아 있다.** 구현된 기능과
현재 실행·검증 가능 상태를 동일하게 해석하지 않는다. 전달 전에 마지막의
[삭제 후 정리 사항](#9-삭제-후-정리-사항)을 확인한다.

## 1. Redis 메시지 소비

담당: [consumer.py](../src/executor_worker/consumer.py)
...
담당: [contracts.py](../src/executor_worker/contracts.py),
[ingress.py](../src/executor_worker/ingress.py),
[store.py](../src/executor_worker/store.py)
...
담당: [outbox.py](../src/executor_worker/outbox.py),
[store.py](../src/executor_worker/store.py)
...
담당: [dispatcher.py](../src/executor_worker/dispatcher.py),
[guard.py](../src/executor_worker/guard.py),
[store.py](../src/executor_worker/store.py)
...
담당: [langgraph_adapter.py](../src/executor_worker/langgraph_adapter.py)
...
담당: [runtime.py](../src/executor_worker/runtime.py),
[config.py](../src/executor_worker/config.py),
[telemetry.py](../src/executor_worker/telemetry.py)
...
담당: [Alembic 가이드](../migrations/README.md),
[store.py](../src/executor_worker/store.py)
...
main.py가 정식 시작 파일이다. 업무 연결은 agent_app.py에서 명시적으로 구현한다.

| 이름 | 제공하는 내용 |
|---|---|
| main.py | 정식 시작, 자원 수명 관리, SIGTERM/SIGINT 종료 신호 처리 |
| agent_app.py | 호스트 그래프 factory와 이벤트별 핸들러 연결 지점 |
| examples/session_graph.py | 이벤트 대기 → 수락 저장 → 반영 → 다음 대기/종료 예제 |
| examples/api_integration.py | 기존 Execution 연결과 그래프 대기 상태 생성 예제 |
| examples/failure_cleanup.py | Executor 취소 요청 후 실제 종료 상태까지 확인하는 선택적 보상 예제 |
| Dockerfile | 워커 런타임·테스트 이미지 빌드 구성 |
| deploy.yaml.example | 같은 Pod의 API+Agent / Worker 배치 예시 |
| migrate-job.yaml.example | 배포 전 DB migration Job 예시 |
| compose.yaml, tests/ | 격리 DB·Redis 기반 검증 구성과 테스트 코드 |

취소 예제는 자동으로 호출되지 않는다. 실행 취소가 필요한 시점은 호스트가
결정한다. 취소 접수만으로 완료를 알리거나 장기 채팅 잠금을 해제하지 않는다.

Dockerfile과 Deployment는 main.py를 실행한다. main은 examples를 import하지 않고
운영 이미지도 examples를 포함하지 않는다. create_graph를 구현하지 않으면 이벤트
소비 전에 오류로 종료한다. 작성 방법은 [Agent 연결 가이드](agent-integration.md)에 있다.

## 9. 삭제 후 정리 사항

확인 기준일 현재 다음 소스 파일은 삭제되어 있다.

- cli.py, dlq_cli.py, trim_cli.py
- dlq.py, stream_maintenance.py
- schema.sql

정식 진입점 작업에서 삭제된 CLI 명령 등록과 문서 안내는 정리했다.
아래 참조는 아직 남아 있으며 이번 작업에서 삭제된 기능을 복원하지 않았다.

| 남은 참조 | 영향·필요한 정리 |
|---|---|
| DLQ·Stream 정리 테스트의 import | 삭제된 모듈에 의존한다. 테스트 수집 시 오류가 발생한다. |
| Store.migrate()의 schema.sql 참조 | 파일이 없다. Alembic 전환에 맞춰 구형 경로를 정리해야 한다. |
| test_migrations.py의 Store.migrate() 호출 | legacy 검증이 삭제된 SQL에 의존한다. |

이전 [검증 기록](../VALIDATION.md)의 전체 테스트 통과 결과를 삭제 후 현재 상태에
그대로 적용하지 않는다. 삭제된 기능과 참조를 정리한 뒤 회귀 검증이 필요하다.
현재 유효 기능의 선별 회귀 결과·제외 항목은 검증 문서에 별도로 기록한다.
...
- [DB 초기화·마이그레이션](../migrations/README.md)
- [기존 검증 기록과 테스트 매핑](../VALIDATION.md)
