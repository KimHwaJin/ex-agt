...
@pytest.mark.postgres
async def test_legacy_schema_matches_revision_without_silent_adoption(
    migration_db,
):
    url, schema = migration_db
    code, output = await alembic("upgrade", "head", url=url)
    ...
    # Only this freshly-created test schema is downgraded/dropped.
    code, output = await alembic(
        "-x",
        "allow_worker_table_drop=true",
        "downgrade",
        "base",
        url=url,
    )
    ...
    async with AsyncConnectionPool(url, open=False) as pool:
        await Store(pool, "legacy-test").migrate()
