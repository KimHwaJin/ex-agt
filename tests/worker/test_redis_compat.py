...
def consumer(redis, stream, handler, mode="compat"):
...
@pytest.mark.redis
async def test_native_mode_rejects_redis_60_without_false_readiness(transport):
    redis, stream = transport
    version = (await redis.info("server"))["redis_version"]
    if tuple(map(int, version.split(".")[:2])) >= (6, 2):
        pytest.skip("Only tests unsupported native commands")  # ty: ignore
    runtime = consumer(redis, stream, RecordingHandler(), "native")
...
@pytest.mark.redis
async def test_mode_switch_keeps_group_and_recovers_existing_pending(
    transport,
):
    redis, stream = transport
    version = (await redis.info("server"))["redis_version"]
    modern = tuple(map(int, version.split(".")[:2])) >= (6, 2)
    modes = ["compat", "native", "compat"] if modern else ["compat"] * 3
    ids = await seed_pending(redis, stream, 3)
    seen = []
    for message_id, mode in zip(ids, modes, strict=True):
        await make_stale(redis, stream, [message_id])
        handler = RecordingHandler()
        runtime = consumer(redis, stream, handler, mode)
        run = asyncio.create_task(runtime.run())
        try:
            await wait_until(lambda h=handler: len(h.messages) == 1)
            seen.extend(item.message_id for item in handler.messages)
        finally:
            await runtime.shutdown()
            await run
    assert seen == ids
