...
정식 소스 편입과 삭제된 운영 도구의 잔여 참조 정리를 완료했다. 공통 Worker는
Agent와 독립이며, 이 저장소의 실제 연결은 2E의 `agent.runtime`에 있다. 현재 단계와
검증 범위는 [전환 계획](../../../docs/worker-centered-refactor.md)을 확인한다.
