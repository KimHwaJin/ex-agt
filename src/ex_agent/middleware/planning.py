...
class PlanModeMismatchError(ValueError):
    """Regenerate a plan; relabeling a partial MULTI plan is not safe."""
...
class PlanOutputMiddleware(PlannerMiddleware):
    ...
    async def aafter_agent(
        ...
        if plan.execution_mode is not context.execution_mode:
            raise PlanModeMismatchError(
                f"Planner returned execution mode {plan.execution_mode}, "
                f"but selected execution mode is {context.execution_mode}. "
                "Regenerate the entire plan for the selected mode; do not "
                "just relabel a partial plan."
            )
...
def _planning_system_message(
    ...
    *,
    ...
    content = (
        ...
        "The execution mode below is authoritative and must be copied "
        "exactly into PlanDraft.execution_mode. Never change it based on "
        "task complexity or revision feedback. "
        "When execution mode is SINGLE, include ALL ordered Steps needed "
        "to complete the requested code/analysis work in this plan. Do not "
        "stop at data preparation or defer planning until after execution. "
        "The final report is generated by the separate reporting phase. "
